...
-- the apps listed here will have their windows moved to the built-in display
property windowPositionsBuiltIn: {¬
  {appName:"Notes"}, ¬
  {appName:"Mail"} ¬
...
property windowPositionsExternal: {¬
  {appName:"Firefox", y:"top", x:"right"}, ¬
  {appName:"iTunes", y:"bottom", x:"right"}, ¬
  {appName:"Safari", y:"top", x:"left"}, ¬
  {appName:"TextWrangler", y:"top", x:"center"}, ¬
  {appName:"BBEdit", y:"middle", x:"center"}, ¬
  {appName:"Terminal", y:"bottom", x:"left"}, ¬
  {appName:"Xcode", y:"bottom", x:"right"} ¬
} -- last item can't have a trailing comma

--(*** LOGIC ***)

-- size of entire desktop, with origin at top left of primary display
property allBounds: {}
...
-- determine scaled resolution of secondary display
set text item delimiters to "UI Looks like:"
if (count of pfl's text items) is 2 then
  set allResolutions to {{item 1 of allResolutions}} & {{word 1 of pfl's text item 2 as number, word 3 of pfl's text item 2 as number}}
end if
set text item delimiters to tid

-- consider the external display as a 3 × 3 grid and use top, middle, bottom, left, center, right to refer to the origin of each square
to makeRelativeValue from relativeTerm on screenNum
  ...
  if relativeTerm = "top" then
    ...
    return (item 2 of item 2 of allResolutions) / 4 as integer
  else if relativeTerm = "bottom" then
    return (item 2 of item 2 of allResolutions) / 2 - 32 as integer
...
set absoluteXY to (makeAbsoluteXY from
